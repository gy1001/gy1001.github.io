...
1. 逻辑原理(同第三节中栈的解题思路一样)：
   ...
   * **当检测到结束标记</时**，将 栈1 的栈顶出栈, 然后 栈2 如果只剩下栈顶，说明到最后了，就返回；否则，就对 栈2 进行弹栈操作 `pop_obj`，判断栈2 栈顶此时不存在`children`属性，就赋值`children`属性为 [], 然后往 栈2 的栈顶`children`属性 `push`刚刚弹出的`pop_obj`,指针移动 tag 的长度+3（因为开始标签的左右尖括号占用两个位置，还有一个斜杠占一个位置） 
