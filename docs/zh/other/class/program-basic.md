...
1. 特点
   ...
   * 多用于国家高科技领域和尖端技术研究
   * 标记它们的运算速度的单位是TFlop/s（1TFlop/s=每秒一万亿次浮点计算） 
2. 截至2018年11月，全世界超级计算机排名前三的见下图。
...
* 不得不提，去IOE行动：I指IBM(服务器提供商)、O指Oracle(数据库软件提供商)、E指EMC(存储设备提供商)。去IOE行动是阿里巴巴在2008年提出的概念，因为IOE代表了高维护费用的存储系统，并且也不够灵活，伸缩性弱
