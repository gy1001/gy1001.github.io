...
1. 实现双向链表：
   1. 实现node类
   2. 实现 DoubleLinkedList 类：pop方法、append(node)方法、append_front(node)方法、remove(node)方法
2. 实现置换算法：
   1. 先进先出算法
      * 把高速缓存看做是一个先进先出的队列
      * 优先替换是最先进入队列的字块
...
问题：可能存在相同频率的情况，这时应该淘汰哪个节点呢？相同频率的节点放入另一个双向链表中，然后同频率节点按FIFO算法淘汰
