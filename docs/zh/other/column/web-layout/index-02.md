...
![img](./assets/cdeb002f45ad4a188983e435151b3967~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![img](./assets/1de13af895c64954acec35c282505e8f~tplv-k3u1fbpfcp-zoom-1.jpeg)

也会在定位元素（显式使用`position` 属性值为非 `static` 的元素）上使用 `z-index` 控制其层叠的顺序（`z` 轴上的层叠顺序），它表示的是用户与屏幕的这条看不见的垂直线：

![img](./assets/25f58e85cf3545a69a4b7e0da0b3dde1~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![img](./assets/52c710846e504b5390bda406de0149b9~tplv-k3u1fbpfcp-zoom-1.jpeg)

当我们的书写方式改变时，块盒子和内联盒子也会有相应的变化，就拿内联盒子为例，书写模式改变之后，它的方向也会随之改变：

![img](./assets/1fa96422579f4ebba966a04a3046c860~tplv-k3u1fbpfcp-zoom-1.jpeg)

简而言之，**块元素遵循流方向，内联元素遵循写入方向**：

![img](./assets/d46de45a34cb4982a4512afd9e8de5c8~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![fig-02-07.jpg](./assets/29035d923f5341289fb3c0527dca42d4~tplv-k3u1fbpfcp-watermark.jpeg)
...
![img](./assets/ac2e7f9fb85644bba2eb64740984ea60~tplv-k3u1fbpfcp-zoom-1.png)

只不过，Web 开发者习惯性以 `width` 、`height` 、 `min-*` 或 `max-*` 以及它们对应的逻辑属性来显式指定一个容器空间的大小：

![fig-02-09.png](./assets/1f03b42bd971488db2132556d5b7102b~tplv-k3u1fbpfcp-watermark.png)

而且容器大小计算方式也会受 CSS 的 `box-sizing` 属性的值影响：

![img](./assets/c4c37c1f5cce489a81ce5e56521dc128~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![img](./assets/9be439c2389c4c03afdb1b6d26e8a249~tplv-k3u1fbpfcp-zoom-1.jpeg)

## 间距

Web 是由很多个元素堆叠而成的，为了让 Web 页面给用户提供更好的体验，Web 设计师在设计时，会根据美学相关的理论来设计元素与元素之间，元素内容与元素盒子边缘之间的间距：

![img](./assets/3f0042bf96b940c3a29b1adb7c6210d2~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![img](./assets/b6d6adba3fbb47aaaf154bebf4e3f770~tplv-k3u1fbpfcp-zoom-1.jpeg)

特别要提出来的是，CSS 中的 `margin` 和 `gap` 表现形式是有较大差异的，它们之间的差异用下图来阐述：

![img](./assets/0ffbb85f59be433d921d03a031188398~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![img](./assets/eb0c5f7661b94655a798afc7b22169c2~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./assets/d7aa755e28f84cc795fd21245db2609d~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![img](./assets/fe03b3367a9d4aba91f4c1e49b8415d9~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./assets/c2a1d5e7540249699d91e3f19fe73b0f~tplv-k3u1fbpfcp-zoom-1.png)

> 上图中左侧是汉语体系的布局效果，右侧是阿拉伯语体系的布局效果。

众所周知，CSS 中有很多属性和值是遵循 **TRBL** (Top、Right、Bottom 和 Left ) 模式的：

![img](./assets/2f128a6f27e746548793726ac7c0f1be~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![img](./assets/bd102861221c42cca2f900a2aad60c34~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![img](./assets/ea763a01c628466f8cdf938c72117d0b~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![img](./assets/561d5783ec354850ae689d61e86ab6fb~tplv-k3u1fbpfcp-zoom-1.png)

而且映射关系与 CSS 的 `writing-mode` 属性值也有关系，对应关系如下图所示：

![fig-02-23.png](./assets/e171d25a3145466bb16a812e4af3bd2c~tplv-k3u1fbpfcp-watermark.png)
...
![img](./assets/ee89006d8eb946a2b9bf5cc24c88752b~tplv-k3u1fbpfcp-zoom-1.jpeg)
...
![img](./assets/9d194cbe7fc845f8be3a6671cfa7fad5~tplv-k3u1fbpfcp-zoom-1.png)

CSS Box Alignment 模块中，取不同的值时，能得到不同的对齐结果，比如靠左（开始）、靠右（结束）、中间对齐、两端对齐等：

![img](./assets/a964cea745874ab18339f42497116af9~tplv-k3u1fbpfcp-zoom-1.jpeg)
